...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args.length)
      return message.reply("UKullanım: / oynatma listesi <YouTube Oynatma Listesi URL'si | Oynatma Listesi Adı>").catch(console.error);
    if (!channel) return message.reply("You need to join a voice channel first!").catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return message.reply("Ses kanalına bağlanamıyor, izinler eksik");
    ...
    message.channel
      .send(
        `${message.author} 📃 Bir oynatma listesi eklendi - **${playlist.title}** <${playlist.url}>
