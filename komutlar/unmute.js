...
const db = require('quick.db')
    if (!message.member.hasPermissions ('KICK_MEMBERS')) return message.channel.send("Yapmak İçin ``KICK_MEMBERS`` Yetkisine Sahip Olmalısın.")
  ...
  if (!modlog) return message.reply('Mod log kanalı ayarlamamışsınız.');
    ...
    if (!user.roles.find(`name`, "Muted")) return message.channel.send('Kişi Mutelenmemiş')
